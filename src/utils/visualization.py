...
def draw_egopath(img, egopath, opacity=0.5, color=(0, 189, 80), crop_coords=None):
    ...
    Args:
        ...
        egopath (list or PIL.Image.Image or numpy.ndarray): Ego-path to be visualized on the image, either as a list of points (classification/regression) or as a mask (segmentation).
    ...
    if isinstance(egopath, list):  # classification/regression
        ...
        if left_rail and right_rail:  # nothing to overlay on an empty path
    ...
    elif isinstance(egopath, (Image.Image, np.ndarray)):  # segmentation
